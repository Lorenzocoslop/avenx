...
if(count($parts)){
    foreach ($parts as $part) {
        $partsAux = explode(' ', $part);
        if(!$objClass->issetParam(trim($partsAux[0]))){
            $order = $Modules['ordenacao'];
            break;
        }
    }
}
...
if($registros > 0){
    ...
    $html .= $Modules['class']::getTable($rs);
    ...
    if($paginas > 1){
        ...
        $html .= '<nav aria-label="List pagination"><ul class="pagination justify-content-center">';

        if($paginas > ($maxPages*2)+1) $html .= '<li class="page-item"><a href="#pagination" class="page-link" onclick="return '.str_replace('#PG',1,$func).'"><i class="ti ti-chevron-left-pipe icon"></i></a></li>';
        if($pagina > 1) $html .= '<li class="page-item"><a href="#pagination" class="page-link" onclick="return '.str_replace('#PG',$pagina-1,$func).'"><i class="ti ti-chevron-left icon"></i></a></li>';
        if($pagina - $maxPages > 1) $html .= '<li class="page-item disabled"><a href="#pagination" class="page-link" onclick="return false">...</a></li>';

        for($i=$inicio; $i<=$fim; $i++){
            $html .= '<li class="page-item '.($i==$pagina?'active':'').'"><a href="#pagination" class="page-link" onclick="return '.($i==$pagina?'false': str_replace('#PG',$i,$func)).'">'.$i.'</a></li>';
        }

        if($pagina + $maxPages < $paginas) $html .= '<li class="page-item disabled"><a href="#pagination" class="page-link" onclick="return false">...</a></li>';
        if($pagina < $paginas) $html .= '<li class="page-item"><a href="#pagination" class="page-link" onclick="return '.str_replace('#PG',$pagina+1,$func).'"><i class="ti ti-chevron-right icon"></i></a></li>';
        if($paginas > ($maxPages*2)+1) $html .= '<li class="page-item"><a href="#pagination" class="page-link" onclick="return '.str_replace('#PG',$paginas,$func).'"><i class="ti ti-chevron-right-pipe icon"></i></a></li>';
