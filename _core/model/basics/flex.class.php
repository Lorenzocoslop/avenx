...
class Flex {
    ...
    public function dbInsert() {
        ...
        $statement = $conn->prepareStatement("INSERT INTO {$this->getTableName(true)} ({$campos}) VALUES ($valores)");
        foreach ($this->params as $var => $value) {
            if ($var != '') {
    ...
    public function dbUpdate() {
        ...
        $statement = $conn->prepareStatement("UPDATE {$this->getTableName(true)} SET {$valores} WHERE {$where}");
        foreach ($this->params as $var => $value) {
            if (!in_array($var, $this->primaryKey) && $var != "dt_cad" && $var != 'usr_cad') {
                switch (strtolower($this->mapper[$var])) {
                    case 'int' : $statement->setInt($var, $value);
    ...
    public static function dbDelete($obj, $where='') {
        ...
        if( trim($where)=='' )
            return false;
        $sql = "DELETE FROM {$obj->getTableName(true)} WHERE {$where}";
    ...
    public static function search($data = [
        ...
        $conn = new Connection();
        $sql = "SELECT {$data['select']} FROM ".$obj->getTableName(true).
