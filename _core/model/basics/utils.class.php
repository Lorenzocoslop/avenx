...
class Utils {
    ...
    {
        return strip_tags(self::replace('/\s\s+/', ' ', html_entity_decode($text, ENT_QUOTES, $GLOBALS['Charset'])));
    }
    ...
    public static function getYoutubeId($url){
        preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/|\/shorts\/)([^"&?/ ]{11})%i', $url, $match);
        if($match){
            return $match[1];
        }
        return '';
    ...
    public static function dateFormat($data, $formato, $lang = 'PT') {
        if (self::dateValid($data)) {
            ...
            $pData = explode(strpos($data, 'T') !== false ? 'T' : " ", $data);
    ...
    public static function dateValid($data) {
        ...
        return checkdate((int)$arrDate['m'], (int)$arrDate['d'], (int)$arrDate['y']);
    }

    //para ativar a minificacao, insira ob_start(); no bootstrap.php e esta funcao no final do index.php do view
    ...
    public static function recursiveCopy(string $sourceDirectory, string $destinationDirectory, string $childFolder = ''){
        ...
        if ($childFolder !== '') {
            ...
            while (($file = readdir($directory)) !== false) {
                ...
                if (is_dir("$sourceDirectory/$file") === true) {
                    self::recursiveCopy("$sourceDirectory/$file", "$destinationDirectory/$childFolder/$file");
                } else {
        ...
        while (($file = readdir($directory)) !== false) {
            ...
            if (is_dir("$sourceDirectory/$file") === true) {
                self::recursiveCopy("$sourceDirectory/$file", "$destinationDirectory/$file");
            }
            else {
