...
class PermissaoUsuario extends Flex {
    ...
    public static function validate() {
        ...
        if($_POST['id_usuario'] == ''){
            throw new Exception('O Usu&aacute;rio n&atilde;o foi informado');
        }
        
        if($_POST['id_permissao'] == ''){
            throw new Exception('A Permiss&atilde;o n&atilde;o foi informada');
        }
    }
    ...
    public static function form($codigo = 0) {
        ...
        $opcoes = ['' => 'Selecione'];
        while ($rs->next()) {
            $opcoes[$rs->getInt('id')] = $rs->getString('nome');
        }
        ...
        $string .= Form::select([
            ...
            $string .= '
                <button type="button" class="btn btn-secondary col-sm-1 addcat px-3" style = "height: 56px" onclick="javascript:modalForm(`permissoes`,0,``, () => updateOptionField(`permissoes`, ``, `#pu_id_permissao`, `nome`));">
