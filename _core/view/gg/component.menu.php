    ...
    <?php if(count($view['menus']) > 0){ ?>
        <li class="text-secondary text-uppercase small nav-item divider fw-bold">Site</li>
    ...
    <?php if($objSession->hasPermition('over.metatags') || $objSession->hasPermition('over.contato') || $objSession->hasPermition('over.parametros')){ ?>
        ...
        <?php if($objSession->hasPermition('over.contato')){ ?>
            <li class="nav-item">
                <a class="nav-link d-flex align-items-center" href="#contato" data-bs-toggle="modal" data-bs-target="#contato">
